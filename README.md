...
LCA-GPT Enterprise คือระบบวิเคราะห์คาร์บอนฟุตพรินต์ในห่วงโซ่อุปทาน โดยใช้ AI อ่านเอกสาร PDF (ใบสั่งซื้อ, รายงานความยั่งยืน) แล้วแปลงเป็นข้อมูล Carbon Audit พร้อม Dashboard แสดงผลแบบ Interactive
...
| AI Model | Groq (primary) / OpenRouter (fallback) |
...
│   └── vector_store.py         # PostgreSQL audit storage + similarity
├── backend/
...
```env
# === LLM (Groq preferred, OpenRouter fallback) ===
GROQ_API_KEY=your_groq_key
GROQ_MODEL=llama-3.3-70b-versatile
OPENROUTER_API_KEY=your_openrouter_key
OPENROUTER_MODEL=poolside/laguna-s-2.1:free

# === Database (required for multi-user + persistence) ===
DATABASE_URL=postgresql://user:password@host/dbname?sslmode=require

# === Search & Intelligence ===
SERPER_API_KEY=your_serper_key

# === Secret database viewer (/db-view) ===
SECRET_DATA_KEY=your_secret_key
```
...
|---|---|---|
| `POST` | `/api/auth/register` | สมัครบัญชี (ผู้ใช้คนแรกได้รับ legacy audits) |
| `POST` | `/api/auth/login` | เข้าสู่ระบบ (Bearer token) |
| `GET` | `/api/auth/me` | ตรวจสอบ session |
| `POST` | `/api/analyze` | อัพโหลด PDF/Excel → AI วิเคราะห์คาร์บอน |
| `GET` | `/api/audits` | รายการ audits ของผู้ใช้ (ใหม่สุดก่อน) |
| `GET` | `/api/audits/meta` | id + วันที่ของ audits (สำหรับลบ/จัดการ) |
| `DELETE` | `/api/audits` | ลบ audits ทั้งหมดของผู้ใช้ |
| `DELETE` | `/api/audits/{id}` | ลบ audit รายการเดียว |
...
| `POST` | `/api/chat` | AI chat จาก audits ของผู้ใช้ |
| `GET` | `/api/secret/audits` | ดูข้อมูลทั้งหมด (ต้องใช้ SECRET_DATA_KEY) |
...
5. **Past Audits** — ดู audit ที่เคยทำก่อนหน้า (ลบรายการที่ไม่ต้องการได้)
6. **ถาม Chat** — สอบถามข้อมูลเชิงลึกจาก audits ของบัญชีคุณ
